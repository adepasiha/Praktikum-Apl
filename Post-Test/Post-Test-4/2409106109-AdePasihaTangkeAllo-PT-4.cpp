...
// Prosedur untuk menampilkan lagu secara rekursif
...
// Fungsi login
bool login(User users[], int userCount, string username, string nim, int &isAdmin) {
...
int main() {
    ...
    while (registerChoice == 'y' && userCount < MAX_USERS) {
        ...
        users[userCount].isAdmin = (users[userCount].username == "adeganteng" && users[userCount].nim == "109") ? 1 : 0;
        userCount++;
